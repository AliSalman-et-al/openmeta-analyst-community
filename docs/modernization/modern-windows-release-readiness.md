...
1. Download `OpenMetaAnalyst-modern-windows-x64.zip` from the modern Python workflow artifact.
2. Extract the ZIP to a writable folder.
3. Run `LaunchOpenMetaAnalyst.bat`.

The launcher starts `OpenMetaAnalyst.exe` and opens the bundled `sample_data\amino.oma` dataset so the standard binary analysis GUI slice can be checked immediately.
...
- `.oma`: representative existing `.oma` files must open without user-visible migration steps, and selected project files must round-trip successfully before release.
