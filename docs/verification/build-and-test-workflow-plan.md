...
This plan turns the verification path optimization decisions into small, reviewable implementation slices. It is governed by [ADR 0078](../adr/0078-split-modern-ci-verification-from-packaging.md), [ADR 0079](../adr/0079-audit-modern-tests-before-restructuring-ci.md), [ADR 0080](../adr/0080-fail-closed-on-ci-r-evidence.md), and the [Test Taxonomy & Audit](test-taxonomy-audit.md).
...
- Rebuild the pytest verification workflow around evidence, runtime, dependencies, and lane ownership.
...
- Tests now live under runtime/evidence directories such as `tests/python/fast`, `tests/analysis_regression`, `tests/python/gui`, `tests/r_stack`, and `tests/packaging`.
...
- Updated `scripts/verify-fast.ps1`
- Updated `scripts/verify-fast.sh`
- Updated `.github/workflows/fast-verification.yml`
...
- Configurable CRAN repository policy in `scripts/install-r-deps.R`
...
- Updated `docs/verification/test-taxonomy.json` decisions for rewrite/merge/remove candidates.

Steps:

1. Start with `tests/packaging/contract/test_windows_distributable_contract.py`.
...
- Lane/ownership directories under `tests`.
