...
Adopt `uv` for the Python Verification Environment as soon as the dependency feasibility spike identifies a viable dependency set. `uv` adoption should not precede proof that the R Stack shape works, but it should be the first environment hardening step after that proof.
