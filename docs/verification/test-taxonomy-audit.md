...
Manual Package Qualification remains available by dispatch. Release tags never
build packages. Packaging-relevant pull requests validate the delivery state
machine and workflow contracts; production candidates are built explicitly
from a full protected-master SHA.
...
After the manifest is accurate, reorganize `tests` into lane and ownership directories such as `fast`, `gui`, `r_stack`, `golden`, `packaging_contract`, and `packaged_smoke`. Markers remain the selectable execution mechanism; directories make ownership, review scope, duplicate coverage, and safe parallelism boundaries visible.
...
- pytest-xdist is enabled only for the required fast feedback directories: `tests/python/fast`, `tests/analysis_regression/golden`, and `tests/packaging/contract`. GUI, R Stack, and packaged smoke evidence remain serialized unless a future isolation review proves they are safe to parallelize.
