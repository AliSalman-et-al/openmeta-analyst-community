...
- User-authored plot headings and axis labels have a generous 80-character ceiling at both customization surfaces and the renderer boundary. Study names remain unrestricted source data and stay complete in tables and projects, while plot-only labels use a 72-character middle ellipsis that preserves both the identifying prefix and trailing year/suffix. Numeric inputs, raw counts, and effect-size values are never subject to text-length limits.
