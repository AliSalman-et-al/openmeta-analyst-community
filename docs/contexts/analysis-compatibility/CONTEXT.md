...
**Plot Options Surface**:
The shared user-facing controls for configuring Plot Artifact presentation. In the Method & Parameters dialog it sets global defaults for plots generated by the next Results output; in the per-plot editor it changes only the selected Plot Artifact.
_Avoid_: Forest-only settings, duplicated plot preferences

**Plot Capability Descriptor**:
The explicit metadata attached to each Plot Artifact by the Analysis Adapter, declaring its plot kind, editability, styleability, composition, and regeneration contract. Every Plot Artifact must have a valid descriptor; display titles are not a source of plot behavior.
_Avoid_: Plot-title inference, GUI plot-name allowlist
