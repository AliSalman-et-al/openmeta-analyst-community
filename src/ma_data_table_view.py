...
class MADataTable(QtWidgets.QTableView):
    ...
    def _make_context_menu(self):
        def _context_menu(event):
            ...
            _connect_action(action, lambda: self.main_gui.delete_study(study, study_index=study_index))
    ...
    def header_context_menu(self, pos):
        ...
        if column_clicked == 0:
            ...
            _connect_action(action_rename, lambda: self.main_gui.edit_group_name(corresponding_tx_group))
    ...
    def paste_contents(self, upper_left_index, source_content):
        ...
        if isinstance(source_content[-1], list) and len(" ".join(source_content[-1])) == 0:
