        ...
        for study in self.studies:
            ...
            for g1 in group_names:
                ...
                for g2 in [group for group in group_names if group != g1]:
                    if (
                        self.ma_unit_has_edge_between_groups(ma_unit, [g1, g2])
                        and not (g1, g2) in adjacency_list
                        and not (g2, g1) in adjacency_list
                    ):
    ...
    def cmp_studies(
        ...
        if directions_to_ma_unit is not None:
            ...
            (
                ...
                group_str,
                outcome_type,
            ) = [val_if_key_in_dict(x, directions_to_ma_unit) for x in keys]
...
class Study:
    ...
    def add_outcome(self, outcome, follow_up_name="first", group_names=None):
        ...
        self.outcomes_to_follow_ups[outcome.name][follow_up_name] = MetaAnalyticUnit(
...
class MetaAnalyticUnit:
    ...
    def calculate_display_effect_and_ci(
        ...
        if check_if_necessary:
            if self._should_calculate_display_effect_and_ci_and_se(
