...
from rc_metastudio.analysis_dataset import Dataset, MetaAnalysisUnit, Study
...
def ensure_analysis_unit(
    ...
    outcome: str,
    follow_up: str,
    tx_groups: Sequence[str],
) -> MetaAnalysisUnit:
    """Return the selected unit, creating missing structure for new studies."""
    if outcome not in study.outcomes_to_follow_ups:
        study.add_outcome(dataset.get_outcome_obj(outcome), group_names=dataset.get_group_names())
    if follow_up not in study.outcomes_to_follow_ups[outcome]:
    ...
    for tx_group in tx_groups:
        if tx_group not in analysis_unit.get_group_names():
            analysis_unit.add_group(tx_group)
...
def to_calculation_scale(
    ...
    if data_type == BINARY:
        return bridge.binary_convert_scale(value, effect, convert_to="calc.scale", n1=n1)
    if data_type == CONTINUOUS:
...
def calculate_raw_effects(
    ...
    if data_type == BINARY:
        ...
        if effect in BINARY_TWO_ARM_METRICS:
            result = bridge.effect_for_study(
                e1, n1, e2, n2, metric=effect, conf_level=conf_level
            )
        else:
            result = bridge.effect_for_study(
                e1, n1, two_arm=False, metric=effect, conf_level=conf_level
            )
        triplet = (
            (None, None, None)
            if result is None
    ...
    if data_type == DIAGNOSTIC:
        ...
        results = bridge.diagnostic_effects_for_study(
            tp, fn, fp, tn, metrics=DIAGNOSTIC_METRICS, conf_level=conf_level
        )
        return {
...
def has_entered_data(analysis_unit: MetaAnalysisUnit) -> bool:
    return any(
        value not in EMPTY_VALS
        for group in analysis_unit.tx_groups.values()
        for value in group.raw_data
    ) or any(
        value not in EMPTY_VALS
        for effect_groups in analysis_unit.effects_dict.values()
...
def has_study_entered_data(study: Study) -> bool:
    ...
    if study.year not in EMPTY_VALS and study.year != 0:
        return True
    if any(value not in EMPTY_VALS for value in study.covariate_dict.values()):
        return True
    return any(
...
def included_studies_have_raw_data(
    ...
    for index, study in enumerate(studies[:-1]):
        if study.include and not raw_data_is_complete(raw_data_for_study(index, one_arm)):
            return False
    return True
