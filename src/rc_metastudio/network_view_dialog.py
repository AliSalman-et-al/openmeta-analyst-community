...
if TYPE_CHECKING:
    import ui_network_view_dialog as _ui_network_view_dialog
else:
    from rc_metastudio.forms import ui_network_view_dialog as _ui_network_view_dialog


class NetworkViewDialog(QDialog, _ui_network_view_dialog.Ui_NetworkViewDialog):
    viewportRefitApplied = pyqtSignal()

    def __init__(self, model, parent=None):
        super(NetworkViewDialog, self).__init__(parent)
        self.setupUi(self)
        self.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, True)
        adaptive_window.register_adaptive_window(
            self, adaptive_window.WindowRole.NETWORK_VIEW
        )
        settings.restore_network_view_dialog_placement(self)
    ...
    def graph_network(self, outcome, follow_up):
        ...
        img_path = r_bridge.dataset_to_simple_network(
    ...
    def _fit_network_to_viewport(self):
        ...
        if (
            ...
            return
        self.network_view_dialoger.resetTransform()
        # layout-audit: allow=intrinsic-ratio; reason=scene follows its intrinsic-ratio visual artifact
        self.network_view_dialoger.fitInView(item, Qt.AspectRatioMode.KeepAspectRatio)
