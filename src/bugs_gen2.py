...
def generate_model_file_binary(
    out_path, metric="or", cr="none", num_covs=0, is_two_group=False
):
    if not is_two_group:
    ...
    if num_covs:
        ...
        out_str = out_str.replace(
            "mu[i] ~ dnorm(mu.bar, invtau2)",
            "mu[i] ~ dnorm(mu.bar.star[i], invtau2)\n\t\tmu.bar.star[i]<-mu.bar + "
            + cov_str,
        )
    ...
    if metric == "rr":
        ...
        out_str = out_str.replace(
            "cr.bar <- exp(mu.bar) / (exp(mu.bar) + 1)", "cr.bar <- exp(mu.bar)"
        )
    ...
    if num_covs:
        ...
        out_str = out_str.replace(
            "d[i] ~ dnorm(delta,invtau2)",
            "d[i] <- d.star[i] + beta*(mu[i] - mu.bar) + beta1*((mu[i] - mu.bar) * (mu[i] - mu.bar)) "
            + cov_str
            + "\n\t\td.star[i] ~ dnorm(delta,invtau2)",
        )
...
def build_reg_str_for_covs(num_covs):
    return "+".join(
        ["cov%s * (x%s[i] - mean(x%s[]))" % (i, i, i) for i in range(num_covs)]
    )
...
def generate_data_file_binary_one_group(
    out_path, num_studies, r, n, prior_on_q, prior_on_lam, covariates, precnorm=0.000001
):
    """
    r -- numerator (events)
    ...
    for cov_index in range(covariates.GetLength(1) - 1):
        all_covs.append(
            build_brugs_list_from_vector(
                "x%s" % cov_index, get_col(covariates, cov_index + 1)
            )
        )
    ...
    f.write(
        ", precnorm=%s, gamma.a=%s, gamma.b=%s, N=%s)"
        % (precnorm, prior_on_q, prior_on_lam, num_studies)
    )
    f.close()


def generate_all_data_files_diagnostic(
    out_dir, num_studies, tps, fps, fns, tns, q_priors, lam_priors, xt
):
    num_diag_outcomes = 8
    metric_functions_one_grp = [
        "specificity(tn, fp)",
        "sensitivity(tp, fn)",
        "ppv(tp, fp)",
        "npv(tn, fn)",
    ...
    for metric_index in range(len(metric_functions_one_grp)):
        ...
        generate_data_file_binary_one_group(
            out_dir + metric_f.split("(")[0] + "//data.txt",
            num_studies,
            r,
            n,
            q_priors[metric_index],
            lam_priors[metric_index],
            xt,
        )

    metric_functions_two_grp = [
        "dor(tp, fp, fn, tn)",
        "lrp(tp, fp, fn, tn)",
        "lrn(tp, fp, fn, tn)",
    ]
...
def generate_data_file_binary(
    ...
    f.write(
        ", precnorm=%s, gamma.a=%s, gamma.b=%s, gamma.c.a=%s, gamma.c.b=%s, N=%s)"
        % (
            precnorm,
            priors_on_q[0],
            priors_on_lam[0],
...
def generate_all_inits_for_binary(outdir, N, num_chains=3):
    ...
    scalar_chains = generate_inits_for_vars_that_need_scalars(
        num_chains, ["delta", "mu.bar", "invtau2", "invtauc2"], [0.0, 0.0, 0.1, 0.05]
    )
    # now comine them
...
def generate_all_inits_for_cont(outdir, N, cr, num_covs, is_two_group, generic=False):
    ...
    list_chains = None
    if is_two_group:
        list_chains = generate_inits_for_vars_that_need_list(
            num_chains, ["delta.star", "theta.c"], N
        )
    ...
    for cov in range(num_covs):
        ...
        scalar_vals.append(0.0)
    scalar_chains = generate_inits_for_vars_that_need_scalars(
        num_chains, scalar_vars, scalar_vals
    )
    for list_inits, scalar_inits in zip(list_chains, scalar_chains):
        list_inits.append(scalar_inits)
...
def generate_all_inits_using_values(
    phi_mat, xi_mat, outdir, num_chains, num_covs, cr, is_two_group
):
    all_chains = massage_inits_to_bugs_format_binary(
        num_chains, phi_mat, xi_mat, num_covs, cr, is_two_group
    )
...
def massage_inits_to_bugs_format_binary(
    num_chains, phi_mat, xi_mat, num_covs, cr, is_two_group
):
    """
    ...
    if is_two_group:
        xi_parameter_names.append(
            "d"
        )  # this will probably need to be changed for other (non-binary) datatypes
    ...
    for chain in range(xi_mat.GetLength(2)):
        ...
        for param in range(xi_mat.GetLength(1)):
            ...
            cur_param_vector = [
                xi_mat[study, param, chain] for study in range(number_of_studies)
            ]
            chain_str.append(
                build_brugs_list_from_vector(
                    xi_parameter_names[param], cur_param_vector
                )
            )
        phi_vals_for_cur_chain = [
            phi_mat[param, chain] for param in range(phi_mat.GetLength(0))
        ]
        chain_str.append(
            ", ".join(
                [
                    "%s = %15.10f" % (parameter, val)
                    for parameter, val in zip(
                        phi_mat_parameter_names, phi_vals_for_cur_chain
                    )
                ]
            )
        )
    ...
    for chain in range(num_chains):
        all_chains.append(
            ", ".join(
                [
                    "%s = %15.10f" % (parameter, val)
                    for parameter, val in zip(parameters, init_vals)
                ]
            )
        )
    return all_chains
    ...
    for chain_i in range(num_chains):
        ...
        for p in parameters:
            if chain_i < 3:
                ...
                val = 0.1 * rand.NextDouble()
                if rand.NextDouble() > 0.5:
                    val = -1 * val
            chain_str.append(
                build_brugs_list_from_vector(
                    p, ["%15.10f" % val for i in range(num_vals_needed)]
                )
            )
...
def build_results_list_for_cont_bayes(results_dir, N, num_covs, nphi, cr, is_two_group):
    ...
    for i in range(len(vars_for_overall_table)):
        insert_arr_into_mat(
            ...
            ma_format_arr(
                get_first_line_arr(
                    results_dir + "//%s//buffer.txt" % vars_for_overall_table[i]
                )
            ),
        )
...
def build_results_list_for_binary_bayes(
    results_dir, N, num_covs, nphi, cr, is_two_group
):
    """
...
def build_diagnostic_bayes_outcome_table(
    results_dir, outcome, N, cr, num_covs, two_group
):
    ...
    for row_index in range(len(lines)):
        row = lines[row_index]
        insert_arr_into_mat(
            outcome_table, row_index, ma_format_arr(parse_numbers_from_line(row))
        )
        ...
        insert_arr_into_mat(
            ...
            ma_format_arr(
                get_first_line_arr(
                    results_dir
                    + outcome
                    + "//%s//buffer.txt" % vars_for_overall_table[i]
                )
            ),
        )
...
def build_results_list_for_diagnostic_bayes(
    results_dir, metrics, N, num_covs, nphis, cr
):
    ...
    for outcome in metrics:
        ...
        if outcome in ["dor", "lrp", "lrn"]:
            is_two_group = True
        cur_outcome_table = build_diagnostic_bayes_outcome_table(
            results_dir, outcome, N, cr, num_covs, is_two_group
        )
        ...
        lowers, means, uppers = (
            System.Array.CreateInstance(float, N),
            System.Array.CreateInstance(float, N),
            System.Array.CreateInstance(float, N),
        )
        ...
        if is_two_group:
            ...
            # overall_var = "delta"
        overall_arr = ma_format_arr(
            get_first_line_arr(results_dir + outcome + "//%s//buffer.txt" % overall_var)
        )
    ...
    if is_two_group:
        ...
        if cr == "none":
            ...
            if num_covs:
                ...
                out_str = out_str.replace(
                    "mu[i] <- delta + beta0 *(theta.c[i] - muc)", "mu[i] <- delta"
                )
        else:
            if num_covs:
                out_str = out_str.replace(
                    "mu[i] <- delta + beta0 *(theta.c[i] - muc)",
                    "mu[i] <- delta + beta0 *(theta.c[i] - muc)" + cov_str,
                )
                out_str += build_assignments_for_covs(num_covs)
...
def generate_data_file_cont_generic(
    out_path,
    num_studies,
    yt_vec,
    yt_var_vec,
...
def generate_data_file_cont_one_group(
    out_path,
    num_studies,
    yt_vec,
    sdt_vec,
