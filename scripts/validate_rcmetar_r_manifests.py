"""Validate RCMetaR R dependency and drift manifests."""
...
def validate_dependency_manifest(manifest: dict) -> list[str]:
    ...
    if manifest["manifest"] != "RCMetaR-r-dependencies":
    ...
    if package_metadata["package"] != "RCMetaR":
        raise ValidationError(
            f"{DEPENDENCY_MANIFEST}: package_metadata.package must be RCMetaR"
        )
    ...
    direct_dependencies = require_list(
        manifest["direct_RCMetaR_dependencies"],
        f"{DEPENDENCY_MANIFEST}:direct_RCMetaR_dependencies",
    )
    scopes_present = {
        scope
    ...
    if not direct_dependencies:
        raise ValidationError(
            f"{DEPENDENCY_MANIFEST}: direct_RCMetaR_dependencies must not be empty"
        )

    direct_names: list[str] = []
    seen_direct: set[str] = set()
    for index, dependency in enumerate(direct_dependencies):
        label = f"{DEPENDENCY_MANIFEST}:direct_RCMetaR_dependencies[{index}]"
    ...
    if overlap:
        raise ValidationError(
            f"{DEPENDENCY_MANIFEST}: dependencies must be separated between direct RCMetaR and app bundle: "
            + ", ".join(sorted(overlap))
        )
    ...
    for package in ("metabook", "CompQuadForm"):
        ...
        if (
            ...
            or app_matches[0].get("source") != "cran"
        ):
            raise ValidationError(
...
def validate_drift_manifest(manifest: dict) -> None:
    ...
    if manifest["manifest"] != "RCMetaR-statistical-drift":
        raise ValidationError(
            f"{DRIFT_MANIFEST}: manifest must be RCMetaR-statistical-drift"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.report_installed_versions:
            ...
            print("validated RCMetaR R dependency and drift manifests")
