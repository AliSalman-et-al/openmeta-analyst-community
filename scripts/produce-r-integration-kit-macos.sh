...
"$rscript" -e 'install.packages(commandArgs(TRUE)[1], lib=commandArgs(TRUE)[2], repos=NULL, type="source", dependencies=FALSE)' --args "$rc_package" "$library" 2>&1 | tee "$logs/rcmetar.log"
