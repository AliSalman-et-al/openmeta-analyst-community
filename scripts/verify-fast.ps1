...
try {
    ...
    Write-Step "Checking verification test taxonomy"
    $taxonomyArgs = @("run", "python", "scripts\validate_test_taxonomy.py")
    if ($StrictTaxonomy) {
        $taxonomyArgs += "--strict"
    }
    uv @taxonomyArgs
    if ($LASTEXITCODE -ne 0) { throw "Verification test taxonomy validation failed." }

    Write-Step "Running parallel fast verification pytest lanes"
    $fastPytestArgs = @("run", "pytest", "tests\python\fast", "tests\analysis_regression\golden", "tests\packaging\contract")
